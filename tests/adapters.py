...
def run_embedding(
    ...
    """
    emb = Embedding(num_embeddings=vocab_size, embedding_dim=d_model, weights=weights)
    output = emb.forward(token_ids=token_ids)
    return output
...
def run_swiglu(
    ...
    output = swiglu.forward(x=in_features)
    return output
...
def run_rope(
    ...
    output = rope.forward(x=in_query_or_key, token_positions=token_positions)
    return output
...
def run_rmsnorm(
    ...
    """
    rms = RMSNorm(d_model=d_model, eps=eps, weights=weights)
    result = rms.forward(x=in_features)
    return result
...
def run_save_checkpoint(
    ...
    Args:
        ...
        model_config (dict | None): Optional model configuration to serialize.
    """
    save_checkpoint(model, optimizer, iteration, out, model_config)
...
def run_load_checkpoint(
    ...
    the checkpoint, and optionally the model_config.
    ...
    Returns:
        tuple[int, dict | None]: the previously-serialized number of iterations and model_config (if present).
    """
    iteration, model_config = load_checkpoint(src, model, optimizer)
